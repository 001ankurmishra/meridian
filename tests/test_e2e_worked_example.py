...
def _find_rahul_fixture() -> tuple[uuid.UUID, uuid.UUID, uuid.UUID]:
    ...
    for t in we["transactions"]:
        if t["source_account_id"] == account_id and Decimal(
            str(t["amount"])
        ) == Decimal("980000.00"):
            target_tx = t
            break
...
def _verify_referenced_row(
    ...
    if reference_table == "transactions":
        row = conn.execute(
            text("SELECT 1 FROM transactions WHERE transaction_id = :rid"),
            {"rid": reference_id},
        ).scalar()
        assert row is not None, f"transactions row {reference_id} not found"
    elif reference_table == "document_chunks":
        row = conn.execute(
            text("SELECT 1 FROM document_chunks WHERE chunk_id = :rid"),
            {"rid": reference_id},
        ).scalar()
...
def test_worked_example_end_to_end(
    ...
    if not loader_url or not db_url:
        pytest.skip("LOADER_DATABASE_URL or DATABASE_URL not set")
    ...
    with superuser_engine.connect() as conn:
        ...
        row = conn.execute(
            text("SELECT transaction_id FROM transactions WHERE transaction_id = :tid"),
            {"tid": transaction_id},
        ).scalar()
    ...
    try:
        ...
        with superuser_engine.begin() as conn:
            conn.execute(
                text(
                    "UPDATE cases SET assigned_analyst_id = :aid WHERE case_id = :cid"
                ),
        ...
        assert status == "COMPLETE", f"Orchestration status: {status}"
        ...
        assert len(inv_rows) == 1, f"Expected 1 inv run, got {len(inv_rows)}"
        run_id = inv_rows[0][0]
        ...
        for r in agent_rows:
            assert r[1] == "SUCCESS", f"Agent {r[0]} status: {r[1]}"
        ...
        for f_row in finding_rows:
            ...
            cites_alerted = any(
                ev_by_id[eid]["evidence_type"] == EVIDENCE_TYPE_ALERTED_TRANSACTION
                for eid in f_ev_ids
                if eid in ev_by_id
            )
            all_policy = all(
                ev_by_id[eid]["evidence_type"] == EVIDENCE_TYPE_POLICY_CHUNK
                for eid in f_ev_ids
                if eid in ev_by_id
            )
            if cites_alerted:
        ...
        assert len(investigative_finding_ids) == 1, (
            f"Expected 1 INVESTIGATIVE finding, got {len(investigative_finding_ids)}"
        )
        assert policy_finding_count >= 1, "No policy-reference finding found"
        ...
        with app_role_engine.connect() as conn:
            for f_row in finding_rows:
                f_ev_ids = f_row[1]
                assert len(f_ev_ids) > 0, f"Finding {f_row[0]} has empty evidence_ids"
                for eid in f_ev_ids:
                    assert eid in ev_by_id, f"Evidence {eid} not in run"
                    ev = ev_by_id[eid]
                    assert ev["investigation_run_id"] == run_id
                    _verify_referenced_row(
                        conn,
                        ev["reference_table"],
                        ev["reference_id"],
                    )
                    if ev["evidence_type"] == EVIDENCE_TYPE_ALERTED_TRANSACTION:
                        assert ev["reference_id"] == transaction_id
                    if ev["evidence_type"] == EVIDENCE_TYPE_AMOUNT_DEVIATION_INPUT:
                        has_deviation_input = True

        assert has_deviation_input, "No amount_deviation_input evidence found"
        ...
        with app_role_engine.connect() as conn:
            ...
            acct_rows = conn.execute(
                text("SELECT status FROM accounts WHERE customer_id = :cid"),
                {"cid": customer_id},
            ).fetchall()
            for ar in acct_rows:
                assert ar[0] == "active"
        ...
        assert report.evidence_completeness_status == "COMPLETE"
        ...
        assert len(report.applicable_policies) > 0, (
            "applicable_policies is empty — policy classification bug not fixed"
        )
        for p in report.applicable_policies:
            assert p.evidence_type == EVIDENCE_TYPE_POLICY_CHUNK
        ...
        with app_role_engine.connect() as conn:
            for p in report.applicable_policies:
                ...
                assert is_syn is True, (
                    f"Chunk {p.reference_id} not from synthetic document"
                )

        assert len(report.evidence) > 0
        for e in report.evidence:
            assert e.evidence_type != EVIDENCE_TYPE_POLICY_CHUNK, (
                "Policy evidence leaked into report.evidence"
            )

        # Union / disjoint check
        report_ev_ids = {e.evidence_id for e in report.evidence}
        policy_ev_ids = {p.evidence_id for p in report.applicable_policies}
        all_finding_ev_ids: set[uuid.UUID] = set()
        for f in finding_rows:
            all_finding_ev_ids.update(f[1])

        assert report_ev_ids | policy_ev_ids == (all_finding_ev_ids), "Union mismatch"
        assert len(report_ev_ids & policy_ev_ids) == 0, (
        ...
        with app_role_engine.connect() as conn:
            is_syn = conn.execute(
                text("SELECT is_synthetic FROM customers WHERE customer_id = :cid"),
                {"cid": customer_id},
            ).scalar()
        assert is_syn is True
        ...
        trail_agent_names = {ar.agent_name for ar in ir.agent_runs}
        ...
        trail_finding_ids = {f.finding_id for f in ir.findings}
        assert trail_finding_ids == set(db_finding_ids)

        for f_entry in ir.findings:
            assert set(f_entry.evidence_ids).issubset(trail_ev_ids)
        ...
        assert evt.output_summary is not None
        assert evt.output_summary["to_status"] == "CLOSED_APPROVED"
        assert evt.output_summary["from_status"] == "OPEN"
